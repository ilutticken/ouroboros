...
- **Weak points VARY** (`WorldManager.getWeakPoint`, the single source of truth for both the hit test and the Renderer). It returns a 5-cell `{start,end}` grid-aligned range or `null` (a **solid, lethal** wall). Existence AND position are derived from a deterministic FNV-1a hash of the canonical boundary key — so ~45% of Wilds walls are solid and the passable gaps sit at different offsets on each wall (you can't line up one row and smash straight through). It is symmetric (a shared wall reads identically from both rooms) and stable across visits. A **`mainPath`** set guarantees a weak point on every boundary from the Hub to the first Safe Zone (Localhost `[5,0]`), so the world is always traversable; dead-ends are fine because you can always retreat through the wall you came in by. `RoomGenerator` clears an obstacle lane aligned to each wall's weak point so a doorway is never blocked.
...
- Every Architect line in the narrative terminal is written as his **private diagnostic log** — self-directed, never addressed to "you." He mutters, gloats, and inadvertently reveals the very mechanics he's trying to hide (`NarrativeManager.on*`, `Game.architectGuide`, the glitch telegraph). The running joke escalates: at 5 Data he notes "it cannot see ANY of this"; the sub-smash and speed logs leak the max-velocity trick; and at **Localhost `[5,0]`** he realizes, mid-log, that you've been reading all along ("...Oh no. It can see this. It has ALWAYS been able to see this. ...Hello."). Keep this voice for all Architect terminal text.

### Modules & the Module Slot
- **Denny drops his Sector Map** the first time you talk to him (`dennyMapDropped` guard, a `mapitem` NPC). Pick it up and it becomes a **carried module** riding your tail (`carriedModule`), which opens a **Module Slot** — a socket (`moduleSlotX/Y`, bottom-left, `Renderer.drawModuleSlot`). 2-Bit tutorializes: steer your **head** onto the slot and press **SPACE** (`tryInstallModule`) to install it. Installing the map sets `state.unlocked.mapModule`, which is what now gates **2-Bit's route minimap** (was `tailRider`). Slip past Denny → no map → Gate remarks on it. This is the first instance of the design-doc **Modules (§4)** system.
