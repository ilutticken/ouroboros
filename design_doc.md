...
- **PENDING — 2-Bit's inventory (what he sells & why):** his stock should read as a *grifter-broker's* haul, not a generic upgrade shop, and ideally tie into the missing-villager economy (salvage/curses from the emptied sectors, Nibble's cursed goods as a parallel/rival stall). *Decision deferred — flagged by the user, to be designed before Nibble's stall is wired.*
...
### PENDING — 2-Bit's shop inventory (the deferred design decision)
Three shop items are half-wired, and fixing them *right* requires the reserved "what does 2-Bit sell & why" decision — so the audit's fixes here were limited to making the shop **robust** (null-safe, no crashes, hidden rows collapse) rather than redesigning it. Decide before Localhost's shop is player-facing:
- **Manual Override (10 Data, "Hold SHIFT to brake")** — `manualBrake` is set on purchase but read by nothing; there is no Shift handler. Either implement the brake or retire the item.
- **Overclocked Processor (30 Data ×3, "permanently increase movement speed")** — `speedLevel` never feeds the speed/gear system, and because it *costs* score (which is also gear fuel), buying it currently *lowers* your effective gear cap. It's redundant with the gear system; wire it in or retire it.
- **Hitchhiker Module (15 Data)** — vestigial: `tailRider` comes from 2-Bit's questline, so its `!tailRider` reveal gate never fires and its "bite-to-open-shop" verb is unimplemented. Remove, or implement the verb.
