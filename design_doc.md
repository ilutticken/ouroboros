...
- **Rollback Buffer (20)** — a lethal hit costs **10 Data + a mass rewind** instead of ending your run: you keep your Data (−10), stay in the current room (no Hub warp), and re-centre safely (`Game.die` intercept). Diegetically a cheap knockoff of **Cache's** rollback tech. If you can't pay 10, you die for real.
...
### Persistence: refreshd vs Cache (Cache moved UP the queue)
Two distinct systems, often conflated:
- **`refreshd`** (the DRAM-refresh daemon) = the in-fiction reason your **unlocks survive a death** (already true in code — death resets score/length/gear, never unlocks).
- **Cache** (the archivist who "remembered every deleted file, every rolled-back version") = **explicit save/load across sessions** (localStorage). This is now **prioritized early** (right after the registry/content refactor): an incremental game needs saves, and Cache is the diegetic vehicle. The Rollback Buffer item is the in-world "cheap version" of her tech, so meeting her can upgrade it.
