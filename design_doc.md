...
- **Design Rationale:** Weak points (the cracked center of each wall, 5 cells / center ± 2) are *breakable barriers*, not free doorways. Ramming one deals crack damage scaled by gear: base speed or slower does nothing (a non-lethal bonk), higher gears crack it, and a full max-gear (gear 3) hit — or accumulated sub-max hits summing to one — smashes it **Open**. State lives on `WorldManager` (`damageWall`/`getWallDamage`/`brokenWalls`/`wallBreakThreshold = 3`). The Renderer draws a broken wall as an Open black gap and a damaged one as a crack brightening orange→white toward the break.
- The Hub (0,0) is a sealed quarantine: ONLY its right wall is breakable (Beat 4's escape); every other Hub wall is lethal. Every Wilds weak point is breakable in any direction. The hit test (`Game.update`, `midX/Y ± 2·gridSize`) must stay in sync with the Renderer (`gapSize = gridSize·5`).
- **Gotcha:** Ramming a weak point at gear 0 must NOT kill (the old behaviour) — it bonks non-lethally so the player learns "hit it with speed" instead of dying unfairly (AGENTS rule 1). A `_wallBonking` flag throttles the bonk feedback so holding into a wall doesn't spam the sound/hint; it resets whenever the snake actually advances. Solid (non-weak-point) wall segments remain instantly lethal.
- **Gotcha (persistence):** `brokenWalls` and `wallDamage` persist across deaths (physical damage stays); only score/length reset. Breaking NEW walls needs gear ≥ 1, which needs mass (score ≥ 10) — matching the growth-gate progression, so a fresh respawn can't smash on until it has re-grown.

### Gate: The Living Firewall
- Gate tracks the player's Y every move-tick (a vertical goalie), so a straight horizontal run can't bypass him — forcing the encounter (`Game.updateGate`). After it resolves he does **not** vanish: he flees on-screen to a doorway, `breakWall`s it open, and exits, leaving a breach the player can follow into the next sector. A `leaving` flag makes a fleeing NPC non-interactive (no re-triggered dialog, no collision).
